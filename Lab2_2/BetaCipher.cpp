...
inline int BetaCipher::getValidKey(const int k, const std::wstring & s)
{
    if (k<=0)
...
inline std::wstring BetaCipher::getValidOpenText(const std::wstring & s)
{
    std::wstring tmp;
    for (auto c:s) {
        if (isalpha(c)) {
...
inline std::wstring BetaCipher::getValidCipherText(const std::wstring & s)
{
    std::wstring tmp;
    for (auto c:s) {
        if (isalpha(c)) {
